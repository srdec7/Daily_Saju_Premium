...
for (const lang in translations) {
  ...
  // Find the exact block for the language
  const regex = new RegExp(`^(\\s*${lang}:\\s*\\{[\\s\\S]*?tileHour:\\s*'.*?',?)\\s*\\},?$`, 'm');
  const match = code.match(regex);
  if (match) {
    const replacement = `${match[1]}\n    paywallTitle: '${t.paywallTitle}',\n    paywallDesc: '${t.paywallDesc}',\n    paywallBtn: '${t.paywallBtn}',\n  },`;
