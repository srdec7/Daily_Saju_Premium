...
for (let i = 0; i < lines.length; i++) {
  ...
  // Inject translations after tileHour definition
  if (currentLang && line.includes('tileHour:')) {
    ...
    if (t) {
      ...
      console.log('No translation found for lang: ' + currentLang);
    }
    currentLang = null; // reset for next language block
  }
}

fs.writeFileSync(path, processed.join('\\n'));
console.log('Successfully and strictly injected line-by-line.');
